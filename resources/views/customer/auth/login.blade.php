...
<body class="min-h-screen antialiased flex items-stretch justify-center text-slate-800 p-4 md:p-6 relative select-none bg-slate-950 overflow-x-hidden">
        ...
        <section class="w-full md:w-[45%] lg:w-[40%] xl:w-[35%] flex flex-col justify-between p-8 sm:p-12 md:p-14 relative z-20 bg-white min-h-full shrink-0">
            ...
            <header class="flex items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-6 text-xs uppercase tracking-widest font-bold">
                    <a href="{{ route('login') }}" class="pb-1 border-b-2 border-accent-teal text-primary-dark">
                        Sign in
                    </a>
                    <a href="{{ route('register') }}" class="pb-1 border-b-2 border-transparent text-slate-400 hover:text-primary-dark transition-colors">
                        Sign up
                    </a>
                </div>
            </header>
            ...
            {{-- Center Form Box --}}
            <div class="my-auto py-10 space-y-8 animate-fade-scale">
                
                <div class="space-y-2">
                    <h1 class="text-3xl font-black tracking-tight" style="color:#0E3247; letter-spacing:-0.03em;">SIGN IN</h1>
                    <p class="text-xs text-slate-500 font-medium">Silakan masuk menggunakan kredensial akun terdaftar Anda.</p>
                </div>
                ...
                <form id="login-form" action="{{ route('login.submit') }}" method="POST" class="space-y-5">
                    ...
                    <div class="space-y-1">
                        <div class="flex justify-between items-center pl-1">
                            <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest" for="password">
                                Password
                            </label>
                            <a href="{{ route('password.request') }}" class="text-[10px] text-accent-teal hover:text-accent-teal-dark hover:underline font-bold transition-colors">
                                Lupa?
                            </a>
                        </div>
                        <div class="flex items-center capsule-input-container rounded-full px-5 py-3 gap-3 border border-transparent">
                    ...
                    <div class="flex items-center gap-2.5 pl-1">
                        <input type="checkbox" id="remember-me" name="remember-me" 
                               class="h-4 w-4 rounded border-slate-300 bg-slate-50 text-accent-teal focus:ring-accent-teal/40">
                        <label for="remember-me" class="text-xs text-slate-500 font-bold select-none cursor-pointer">Ingat saya untuk sesi berikutnya</label>
                    </div>
    ...
    <script>
        document.getElementById('login-form').addEventListener('submit', function(e) {
            ...
            // Dynamic stages of authentication animation
            setTimeout(() => {
                message.textContent = "Memverifikasi Kredensial Keamanan...";
            }, 600);

            setTimeout(() => {
                // Determine destination dashboard message based on input
                if (emailInput.includes('admin') || emailInput === 'admin') {
                    message.textContent = "Mempersiapkan Portal Administrator...";
                } else {
                    message.textContent = "Mempersiapkan Dashboard Pelanggan...";
                }
            }, 1300);

            // Intentional small delay to play the cinematic loop before submitting
            setTimeout(() => {
                e.target.submit();
            }, 2100);
        });
